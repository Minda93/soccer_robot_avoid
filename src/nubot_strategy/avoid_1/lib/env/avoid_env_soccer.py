...
class NubotAvoidEnv(RobotGazeboEnv):
    ...
    def _Init_Pose(self,reset = 1):
        r""" reset
            reset = 0 -> no reset
            reset = 1 -> only obstacle random - fixed goal and fixed robot
            reset = 2 -> randorm robot and obstacle - fixed goal
            reset = 3 -> all random
            reset = 4 -> only goal random - current robot and current obstacle
            reset = 5 -> only robot random -  current obstacle and current goal
        """
        if(reset == 5):
            """ robot """
            # random
            ...
            self.robot['ang'] = -180.0
            print('random goal {}'.format(self.goal['pos']))
    ...
    def Compute_Reward(self,obs,action,info):
        ...
        """ reward 9 """
        dis_rs = np.hypot(*(np.array(self.robot['pos'][:2]) - self.init_pos))
        t = time.time()-self.start_time

        reward = (-10*dis+(2*dis_rs))/40.0
        # print(reward)
        reward += round(1.274*(1.57-abs(obs[4]-angle)),2)
        # reward /= 50.0
        # print(reward)
        if(info == 'goal' and self.goal_count >= 50):
    ...
    def Cal_Scan_Reward(self,obs,angle,error = 0.8,ang_range = 15):

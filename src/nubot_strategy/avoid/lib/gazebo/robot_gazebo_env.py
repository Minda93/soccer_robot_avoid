    ...
    def Init_Model_States(self,msg):
        ...
        for model in msg.name:
            ...
            if(msg.name[i] == self.__robotName):
                self.robotPos[0] = round(msg.pose[i].position.x,2)
                self.robotPos[1] = round(msg.pose[i].position.y,2)
    ...
    def Sub_Robot_Model(self,msg = None):
        ...
        for model in msg.name:
            if(msg.name[i] == self.__robotName):
                ...
                self.robotVec[0] = round(msg.twist[i].linear.x,3)
                self.robotVec[1] = round(msg.twist[i].linear.y,3)
                # print(self.robotVec)
            i += 1
